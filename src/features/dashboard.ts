...
async function getStats(user: AuthenticatedUser) {
...
async function getCharts(user: AuthenticatedUser) {
  const enquiriesVisible = canSeeEnquiries(user);
...
async function getRecentActivity(user: AuthenticatedUser) {
  const enquiriesVisible = canSeeEnquiries(user);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    return ok(res, 'Dashboard stats fetched successfully', await getStats(req.user!));
  }),
);
...
router.get(
  '/charts',
  asyncHandler(async (req, res) => {
    return ok(res, 'Dashboard charts fetched successfully', await getCharts(req.user!));
  }),
);
...
router.get(
  '/recent-activity',
  asyncHandler(async (req, res) => {
    return ok(res, 'Recent activity fetched successfully', await getRecentActivity(req.user!));
  }),
);
